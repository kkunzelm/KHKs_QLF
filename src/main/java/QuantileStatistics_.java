...
public class QuantileStatistics_ implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
		...
		for (int n = 1; n <= stackSize; n++) {
			...
			// length )
			// Kopiert length viele Einträge des Arrays src ab der Position srcPos in ein
			// Array dest ab der Stelle destPos. Der Typ des Feldes ist egal, es muss nur in
			// beiden Fällen der gleiche Typ sein. Die Methode ist zumindest für große
			// Felder schneller als eine eigene Kopierschleife.
		...
		// determine the number of positive, negative and zero pixels - because I need
		...
		// wäre vielleicht eleganter, n in der nächsten Schleife durch if (pixel not
		// NaN) then n = n + 1 zu bestimmen
		 ...
		 * http://en.wikipedia.org/wiki/Algorithms_for_calculating_variance def
		 * compensated_variance(data): n = 0 sum1 = 0 for x in data: n = n + 1 sum1 =
		 * sum1 + x mean = sum1/n
		 * 
		 * sum_2 = 0 sumc = 0 for x in data: sum_2 = sum_2 + (x - mean)**2 sumc = sumc +
		 * (x - mean) variance = (sum_2 - sumc**2/n)/(n - 1) return variance
		 * 
		 */
