 ...
 * Polygon ein umh�llendes Rechteck,aber nicht Polyline.
 *
 * Ich habe als L�sung die Abfrage nach Polyline einfach auskommentiert.
 *
 * Wichtig: die ROI muss mit einem Polygon markiert werden. Dann klappt es.
 * Alternativ k�nnte man nat�rlich den offset selbst berechnen... aber dazu habe ich im Moment keine Zeit und es klappt ja.
 *
 */
...
public class SubstractTrianglePerc_ implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
        ...
        // KH 23.2.2006: habe bei dieser Abfrage Polyline durch Polygon ersetzt. Jetzt geht das Diff.Bild
		...
		ImagePlus floatImg = NewImage.createFloatImage("Sub Triangle of " + orgImg.getTitle(), width, ip.getHeight(),1,NewImage.FILL_BLACK);
		ImageProcessor floatProcessor = floatImg.getProcessor();
		float[] floatPixels = (float[]) floatProcessor.getPixels();
		
		// KH von mir erg�nzt: soll den Hintergrund mit Float.NaN f�llen, so dass die Statistik-Auswertung wieder stimmt
		// nachteil: ich muss die Routinen Clear, Fill f�r Float.NaN statt Vordergrund/HintergrundFarbe anpassen.
		...
		for(int y=rectRoi.y; y <(rectRoi.y+roiHeight); y++, roiY++) {
			...
			for(x=rectRoi.x, roiX=0; x<(rectRoi.x+roiWidth); x++, roiX++) {
				...
				try{ // just to be sure, not needed (try{} catch{})
					...
					if(shouldBeColor > -1) {	// Pt. is in Polygon
						// diff = (aktColor / shouldBeColor) * 100;    	// w�rde Werte von 0 bis 200 Prozent ergeben (je
					...
					System.err.println("Pt. ("+x+"/"+y+") with index " + i + " is out of bounds (in ROI is ind: " + roiI+")!");
				}
			}
	...
	//	floatImg.show();
		...
		//Calculate which value in the range from 0 to 255 is equivalent to zero in float image
		...
		ImagePlus ci = null;
		/** kh: Ich habe festgestellt, byte Bilder Rechen-Fehler bei der Auswertung aufweisen 
		 *  kh: es spricht nichts dagegen, dass das Differenzbild float bleibt
		 *
		 *	if(isByte) {
			ImageProcessor bp = floatProcessor.convertToByte(true);
			byteImage = new ImagePlus("Sub Triangle of " + orgImg.getTitle(),bp);
			ci = byteImage;
			byteImage.show();
		} else { */
			ci = floatImg;
...
//		 removing roi
	...
	private class PolyTree {
		...
		PolyTree(int[] x, int[] y, int length, ImageProcessor ip) {
			...
			//	System.out.println("C: " + ip.getPixel(x[0],y[0])+"\t"+ip.getPixel(x[1],y[1])+"\t"+ip.getPixel(x[2],y[2]));
		...
		/**
		 * Gets the weight of a point if it's in a triangle
		 * return null if point isn't in polygon
		 */
		float getWeight(int x, int y) {
			if(!polygon.contains(x,y))	//(x/y) NOT in polygon
				return -1;
			float sum;
			if(g != null) {			// is triangle => solving
				float[] weight = g.solve(x,y);
				sum = (weight[0] * colors[0] + weight[1] * colors[1] + weight[2] * colors[2]);
			//	System.out.println("Pt 0 has color " + colors[0] + " and has weight " + weight[0]);
			//	System.out.println("Pt 1 has color " + colors[1] + " and has weight " + weight[1]);
			//	System.out.println("Pt 2 has color " + colors[2] + " and has weight " + weight[2]);
