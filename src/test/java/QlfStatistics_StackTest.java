...
class QlfStatistics_StackTest {
	private QlfStatistics_Stack qlf = new QlfStatistics_Stack();
	...
	void runSingleImage() {
		...
		imagePlus.setImage(createImage());
	...
	@Test
	void runSingleImageFromStack() {

		// Create IJ Image Stack with 32 Bit float Grey Depth
		ImagePlus[] images = {new ImagePlus("1", createImage()), new ImagePlus("2", createImage())};

		ImagePlus imagePlus;
		imagePlus = ImagesToStack.run(images);
		imagePlus.setTitle("Test Image");

		ImageProcessor imageProcessor = imagePlus.getProcessor();

		// suppress dialog
		qlf.showDialog = false;
		qlf.processSingleImage = true;
		qlf.selectedImageIndex = 1;
